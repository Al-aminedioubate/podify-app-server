...
export const updateProfile: RequestHandler = async (req: RequestWithFiles, res) =>{
  ...
  if(!user) throw new Error("Something went wrong, user not foud!");
  ...
  if(avatar){
    //if there is already an avatar file, we want to remove that from the cloud before uploading new one
